...
models = [
        "slate",
        "slate-bcdec (slotsize64, mlpsize128, slotiter7)", # sa
        "slate-bcdec (slotsize192, mlpsize192, slotiter3)", # sa-large
        "iodine",
]
for m_name in models:
    if not m_name in confs["ocrs_for_ari_mse_pp"].keys():
        print(f"model {m_name} is not predefined. Please use in {confs['ocrs'].keys()}.")
        raise ValueError
